...
class Developer(discord.Cog, guild_ids=survey_wolf_bot.config["dev_guilds"]):
    ...
    async def set(self, ctx: discord.ApplicationContext, log: discord.Option(str, description="The Log To Set", choices=logs)):
        w = await ctx.channel.create_webhook(name=f"{self.bot.user.name} {self.log_text[log]} Log",
    ...
    async def unset(self, ctx: discord.ApplicationContext,
